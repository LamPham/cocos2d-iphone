...
-(id) init
{
	if( (self=[super init]) ) {
		...
		[self addChild: ttexture_];
		// run an action for TTexture @see TTexture.h for info
		CCAction *action = [CCTextureMoveBy actionWithDuration:20.f position:ccp(10,0)];
		[ttexture_ runAction:action];
...
-(id) init
{
	if( (self=[super init]) ) {
		...
		// add the label as a child to this Layer
		[self addChild: ttexture_];
		// run an action for CCTransformTexture @see CCTransformTexture.h for info
		CCAction *action = [CCTextureRotateBy actionWithDuration:10.f angle:360.f];
		[ttexture_ runAction:action];
...
-(id) init
{
	if( (self=[super init]) ) {
		...
		[self addChild: ttexture_];
		// run an action for CCTransformTexture @see CCTransformTexture.h for info
		CCAction *action = [CCTextureScaleBy actionWithDuration:10.f scale: 2.f];
		[ttexture_ runAction:action];
